...
class AcceptRequestsTests(APITestCase):
    ...
    def test_valid_accept_request(self):
        ...
        url = reverse('accept-request', kwargs={'id': request.id})
        ...
        self.assertEqual(updated_request.accepted_by, EmailUser.objects.get(id=2))
    ...
    def test_invalid_request_status(self):
        ...
        self.assertEqual(response.json()['message'], "Request Already Fullfilled")
        updated_request = Request.objects.get(id=request.id)
        self.assertNotEqual(updated_request.request_status, "OPEN")

    def test_invalid_request_id(self):
        mock_signup(self.client)
        mock_login(self.client)

        url = reverse('accept-request', kwargs={'id': "12345"})
        response = self.client.post(url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
