...
class PasswordResetVerifyTests(APITestCase):
    def test_password_reset_verify_invalid_code(self):
        url = reverse('password-reset-verify', kwargs={'code': '123123'})
    ...
    def test_password_reset_verify_expired(self):
        ...
        pwd_reset_code.save()
        url = reverse('password-reset-verify', kwargs={'code': pwd_code})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_password_reset_verify_success(self):
        ...
        self.assertEqual(response.json()['message'], 'Request Verified Successful')
        self.assertEqual(PasswordResetCode.objects.filter(code=pwd_code).count(), 1)
